...
public class UnzipUtil {
    ...
    public static Optional<String> getJarPath(File unzipped) {
        ...
        File[] binDirFiles = findSubDir(unzipped, "bin").listFiles(new FilenameFilter() {
            @Override
            public boolean accept(File file, String s) {
                return s.endsWith(".jar");
            }
        });
        if (binDirFiles.length > 0) {
            jarPath = Optional.of(binDirFiles[0].getAbsolutePath());
        }
        return jarPath;
    }

    public static File findSubDir(File parent, final String dirName) {
